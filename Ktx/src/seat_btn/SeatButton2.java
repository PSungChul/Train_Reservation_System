...
public class SeatButton2 extends Frame{
	public SeatButton2() {
	...
	//--------------------------------------------------
	public SeatButton2(Variable va, ChooseSeatMain cs) {
		//프레임 생성
		...
		//예약좌석 체크
		for( int i = 0; i < btn_seat.length; i++ ) {
			if( ("3-" + btn_seat_name[i]).equals(cs.seat_name[2][i]) ) {
					...
					if( e.getComponent().getBackground() == Color.LIGHT_GRAY ) {
						for( int i = 0; i < btn_seat.length; i++ ) {
							if( e.getComponent() == btn_seat[i] ) {
								...
								seat_name[i] = "3-" + btn_seat_name[i];
								cs.seat_name[2][i] = "3-" + btn_seat_name[i];
							}//inner if
						}//inner for
						//좌석선택이 지정숫자보다 많아질 때
