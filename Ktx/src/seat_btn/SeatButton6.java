...
public class SeatButton6 extends Frame{
	public SeatButton6() {
	...
	//--------------------------------------------------
	public SeatButton6(Variable va, ChooseSeatMain cs) {
		//프레임 생성
		Frame f = new Frame("7호차");
		f.setSize(650, 880);
		...
		//예약좌석 체크
		for( int i = 0; i < btn_seat.length; i++ ) {
			if( ("7-" + btn_seat_name[i]).equals(cs.seat_name[6][i]) ) {
					...
					if( e.getComponent().getBackground() == Color.LIGHT_GRAY ) {
						for( int i = 0; i < btn_seat.length; i++ ) {
							if( e.getComponent() == btn_seat[i] ) {
								...
								seat_name[i] = "7-" + btn_seat_name[i];
								cs.seat_name[6][i] = "7-" + btn_seat_name[i];
							}//inner if
						}//inner for
						//좌석선택이 지정숫자보다 많아질 때
